    ...
    public class RunServer
    {
        public static void StartServer(int port)
        {
            ...
            while (true)
            {
                ...
                {
                    Logger.Log(string.Format("Thread[" + Thread.CurrentThread.ManagedThreadId + "]: Server in={0} ({1} KB/s)  out={0} ({1} KB/s)", messagesReceived, (dataReceived * 1000 / (stopwatch.ElapsedMilliseconds * 1024))));
                    stopwatch.Stop();
